...
export default function MediaLibrary() {
  ...
  useEffect(() => {
    if (
      ...
      selectedMedia.url.startsWith('data:application/pdf;base64,')
    ) {
      try {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${BASE_PATH}/api/media/${id}`, {
        method: 'DELETE',
      });
      ...
      if (res.ok) {
        ...
        toast.error(
          errorData?.error || 'Failed to delete media'
        );
      }
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedMediaIds.length} files? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (deletedIds.length > 0) {
        ...
        if (
          selectedMedia &&
          deletedIds.includes(selectedMedia.id)
        ) {
          setSelectedMedia(null);
        }
      }
      ...
      if (successCount < selectedMediaIds.length) {
        toast.error(
          `${
            selectedMediaIds.length - successCount
  ...
  const handleUpdate = async (
    ...
    try {
      const res = await fetch(
        `${BASE_PATH}/api/media/${selectedMedia.id}`,
        {
          method: 'PATCH',
          headers: {
      ...
      if (res.ok) {
        const updatedMedia: Media = await res.json();
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-light text-[#1d2327]">
          Media Library
        </h1>
      ...
      <div
        className={`w-full border-2 border-dashed rounded-lg p-12 text-center mb-8 transition-colors ${
          dragActive
            ? 'border-[#5e3fde] bg-[#f0f6fc]'
            : 'border-[#c3c4c7] bg-white'
        }`}
        onDragOver={(e) => {
          e.preventDefault();
          setDragActive(true);
            ...
            <div
              ...
              className={`relative group bg-white border ${
                selectedMediaIds.includes(item.id)
                  ? 'border-[#5e3fde] ring-2 ring-[#5e3fde]/20'
                  : 'border-[#c3c4c7]'
              } rounded-[3px] aspect-square flex items-center justify-center overflow-hidden cursor-pointer transition-all`}
            >
              <div
              ...
              <button
                ...
                className={`absolute top-2 right-2 z-20 bg-white rounded flex items-center justify-center shadow-sm ${
                  selectedMediaIds.includes(item.id)
                    ? 'text-[#5e3fde] opacity-100'
              ...
              {item.mimeType.startsWith('image/') ? (
                ...
                <div className="flex flex-col items-center justify-center p-4">
                  ...
                  <span className="text-[11px] text-gray-500 text-center break-all line-clamp-2">
                    {item.filename}
                  </span>
                </div>
              )}
          ...
          <div className="bg-white w-full max-w-5xl h-full max-h-[700px] flex rounded-md shadow-xl overflow-hidden relative">
            ...
            <div className="w-80 bg-gray-50 p-6 overflow-y-auto">
              ...
              <form
                ...
                className="space-y-4 text-[13px] border-t border-gray-200 pt-6"
              >
                <div>
