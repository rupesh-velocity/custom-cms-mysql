...
import SearchReplaceSettings from './SearchReplaceSettings';
...
export default function AddonSettingsClient({slug}:{slug:string}) {
  ...
  if(slug==='google-reviews') {
    ...
    const keys=['analytics_head_code','analytics_body_code'];
    content=<>
      ...
      <p className="text-xs text-gray-500 leading-5">If Cookie Consent is enabled, custom tracking snippets load only after the visitor accepts cookies. GA4 can be managed inside your GTM container without adding a separate GA4 loader here.</p>
    </>;
  ...
  } else if(slug==='search-replace') {
    content=<SearchReplaceSettings/>;
  } else if(slug==='smtp') {
