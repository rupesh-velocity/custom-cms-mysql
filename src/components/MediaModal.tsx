...
export default function MediaModal({ isOpen, onClose, onInsert }: MediaModalProps) {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    if (lastUploadedUrl) {
      const newFile = media.find(m => m.url === lastUploadedUrl);
      if (newFile) setSelectedMedia(newFile);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 sm:p-8 font-sans">
      <div className="bg-white w-full max-w-6xl h-full max-h-[800px] flex flex-col rounded-md shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-medium text-gray-900">Add Media</h2>
                      ...
                      >
                        {item.mimeType.startsWith('image/') ? (
                          ...
                          <div className="flex flex-col items-center justify-center h-full p-2">
                            ...
                            <span className="text-[10px] text-gray-500 text-center break-all line-clamp-2">{item.filename}</span>
                          </div>
                        )}
                ...
                {selectedMedia ? (
                  <div className="space-y-4 text-[13px]">
                    {selectedMedia.mimeType.startsWith('image/') && (
                      <img src={selectedMedia.url} alt={selectedMedia.filename} className="w-full h-auto bg-gray-200 mb-4" />
                    )}
                    <div>
                      ...
                      <div className="text-gray-500">{Math.round(selectedMedia.size / 1024)} KB</div>
                    </div>
                  ...
                  <div className="text-gray-500 text-[13px]">Select an item to view its details.</div>
                )}
              </div>
            </div>
          )}
